...
- (void)scrollWheel:(NSEvent *)theEvent {
  RNCTVShengWebView *RNCTVShengWebView = (RNCTVShengWebView *)[self superview];
  RCTAssert([RNCTVShengWebView isKindOfClass:[RNCTVShengWebView class]], @"superview must be an RNCTVShengWebView");
  if (![RNCTVShengWebView scrollEnabled]) {
...
@interface RNCTVShengWebView () <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler,
#if !TARGET_OS_OSX
...
@implementation RNCTVShengWebView
{
#if !TARGET_OS_OSX
  ...
  // https://github.com/xianshengtv/react-native-tvsheng-webview/issues/62
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  if(self.useSharedProcessPool) {
    wkWebViewConfig.processPool = [[RNCTVShengWKProcessPoolManager sharedManager] sharedProcessPool];
  }
...
- (void)layoutSubviews
{
  ...
  // Ensure webview takes the position and dimensions of RNCTVShengWebView
